...
extern const boolean DEBUGCLOCK;
...
public:
	...
	unsigned long clockHighTime = 0;// time in milliseconds of last clock signal (eg for timing pulses and display)
...
private:
	...
	unsigned long lastGoodBPM = 0;	// time in milliseconds since we got a valid BPM reading to allow brief dropouts to be handled
...
int ClockHandler::readClock() {
	...
	if (testClockBPM > 0 && clockSignal && testClockBPM >= minBPM && testClockBPM < maxBPM) {
		...
		previousBPM[(int) counterPrevBPM % avStepsBMP] = testClockBPM;			//	add BPM to averager array using a modulus to shift poisition each clock
	...
	if (DEBUGCLOCK && testClockBPM > 0) {
		Serial.println("Dropped  "); Serial.print(" ms: "); Serial.println(millis());
	}
...
void ClockHandler::printDebug() {
	...
	Serial.print(" in: "); Serial.print(clockInput);
	Serial.print(" bpm: "); Serial.println(clockBPM);
